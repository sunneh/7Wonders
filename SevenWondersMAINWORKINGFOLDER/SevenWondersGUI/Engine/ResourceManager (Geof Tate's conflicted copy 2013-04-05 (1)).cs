...
//As of now no need to call this method. Add notes here if you want to call this and why.
    ...
    */
        ...
        * Input: A Player
        * 
        * Output: void -> Given a player, This is the helper function to resetSpecialResources.
        ...
        */
        ...
        public int[] SpecialResourceArray(string s) {
            ...
            for (int i = 0; i < SResources[s].Count; i++) 
            {
                for (int j = 0; j < 7; j++) 
                { 
                    specialResourceTable[j] += SResources[s][i].getResources()[j]; 
                }
            }
